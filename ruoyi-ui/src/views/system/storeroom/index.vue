...
<script>
import { listStoreroom, getStoreroom, addStoreroom, updateStoreroom, delStoreroom } from "@/api/system/storeroom";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        storeroomname: [
          { required: true, message: "库房名称不能为空", trigger: "change" }
        ]
      }
    };
  },
  ...
  methods: {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const noticeIds = row.storeroomno;
      this.$modal.confirm('是否确认删除库房编号为"' + noticeIds + '"的数据项？').then(function() {
